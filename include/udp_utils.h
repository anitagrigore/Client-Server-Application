...
struct UDPMessage {
  ...
  // Return a std::string containing the message from the udp client.
  std::string str() const {
    ...
    int err = getnameinfo((struct sockaddr *)&source.addr, source.addrlen, host,
    ...
    // add the format, ip address, port and topic to the output string.
    ...
    case FLOAT: {
      ...
      sprintf(buf, "- FLOAT - %s%d.%0*d", message.float_msg.sign ? "-" : "",
              integral, modulo, fractional);
      output.append(buf, strlen(buf));
      break;
    }
    case STRING: {
...
// If the size of the message is smaller than the size of the preamble,
// can't parse it.
